...
export async function generateAlias(
	...
	const { data } = await axios.post<[{ type: string; msg: string[] }]>(
		...
		{
			address: generateEmail(settings, hostname),
		},
	);
	...
	return {
		...
		targetAddress: alias.data.goto,
		address: alias.data.address,
		active: alias.data.active === alias.data.active_int,
		// mailcow doesn't return a valid js date for some reason (https://github.com/mailcow/mailcow-dockerized/issues/4876)
		created: new Date(`${alias.data.created}.000Z`),
		modified: alias.data.modified
			? new Date(`${alias.data.modified}.000Z`)
			: null,
		siteHash: alias.data.private_comment.split("_")[1],
	};
}
...
export async function fetchAliases(
	...
	return data
		.filter(
			...
			(alias) => alias.private_comment.startsWith("aliasextension_"),
		)
		.map((alias) => {
			return {
				id: alias.id,
